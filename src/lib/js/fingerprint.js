...
// Check Session
var sessionLocate = sessionStorage.getItem('NsGeoLocation');
//console.info('sessionLocate', sessionLocate);
if (sessionLocate === 'null' || sessionLocate === null) {
    sessionLocate = '{}';
}
//console.log(sessionLocate);
var currentDate = new Date();
var sessionObject = JSON.parse(sessionLocate);
var expirationDate = sessionObject.expiresAt;
geolocation = sessionObject.geolocation;
if (Date.parse(currentDate) < Date.parse(expirationDate) && sessionObject.geolocation !== null) { // Ultima verificação ainda esta ativa, utilizar e ignorar a chamada
    //setTimeout(function () {
    //geolocation = sessionObject.geolocation;
    //}, 1000);
} else {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
            ...
            // Salvar na sessão para evitar várias chamadas
            var d1 = new Date(), d2 = new Date(d1);
            //d2.setMinutes(d1.getMinutes() + ((fp.isMobile()) ? 2 : 240));
            d2.setMinutes(d1.getMinutes() + 1);
            var sessionObjectOrigin = {
                expiresAt: d2,
...
function NSUtilFingerprint(force) {
    ////console.info('fp-data', fingerprint);
    return fingerprint;
}
